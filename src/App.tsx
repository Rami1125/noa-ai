...
export default function App() {
  ...
  return (
    <div className="h-screen w-full flex bg-[#F8FAFC] font-['Heebo'] rtl" dir="rtl">
      {view === "admin" ? (
        <AdminDashboard 
          userId={userId || ""} 
          specId={SPEC_APP_ID} 
        ...
        <div className="flex-1 flex flex-col bg-[#e5ddd5] bg-opacity-40 relative overflow-hidden" 
          ...
          <header className="h-20 bg-[#1E293B] text-white flex items-center px-6 justify-between shadow-xl z-20">
            ...
            <AnimatePresence>
              {messages.map(msg => (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`p-1 shadow-sm relative mb-2 max-w-[95%] sm:max-w-[85%]
                    ${msg.sender === "noa" ? "bg-white rounded-[24px] rounded-tl-none border-r-4 border-[#C5A059] w-full" : "bg-[#DCF8C6] rounded-[24px] rounded-tr-none px-4 py-2"}`}>
                    
                    {msg.sender === "noa" ? (
                      <div className="noa-render p-4" dangerouslySetInnerHTML={{ __html: msg.text }} />
                    ) : (
                      <div className="text-[16px] leading-relaxed py-1">{msg.text}</div>
                    )}
                    
                    <div className="flex justify-end gap-1 mt-1 text-[10px] text-slate-400 font-bold px-2 pb-1">
          ...
          <footer className="bg-white/90 backdrop-blur-[10px] p-4 flex items-center gap-3 border-t border-slate-200">
            <Smile className="text-slate-400" />
            <Paperclip className="text-slate-400 rotate-45" />
            <input 
              value={inputText}
            ...
            />
            <button onClick={handleSendMessage} className="w-12 h-12 bg-[#1E293B] text-white rounded-full flex items-center justify-center shadow-lg">
      ...
      <style>{`
        ...
        .noa-render .card { background: white; border: 1px solid #e2e8f0; padding: 20px; border-radius: 24px; margin: 10px 0; box-shadow: 0 10px 15px -3px rgb(0 0 0 / 0.1); backdrop-filter: blur(10px); }
